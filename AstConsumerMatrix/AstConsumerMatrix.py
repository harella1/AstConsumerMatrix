...
def output(calls):
    ...
    for item,method in d[pattern].Interfaces:
        Interfaces[pattern][y.index(method),x.index(item)] = 1
    ...
    the_table = ax2.table(cellText=Dependencies[pattern], loc='center',cellLoc='center',rowLabels=y, colLabels=x,cellColours=plt.cm.hot(vals))
